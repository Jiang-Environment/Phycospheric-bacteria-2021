library(vegan)
asvmat <- read.csv("C:/Users/mumua/Desktop/16s paper/PCoA/ASV_table_PCoA_BW.csv", header=TRUE, quote="", row.names=1)
asvmat <- data.frame(t(asvmat)) #转置
sampledata <- read.csv("C:/Users/mumua/Desktop/16s paper/PCoA/sample_data_PCoA _BW.csv", header=TRUE, quote="", row.names=1)
...
fig <- ggplot(asv.plot, aes(x=Axis.1,y=Axis.2,fill=group))+ 
  ...
  stat_ellipse(type = "norm",level = 0.95,linetype = 2, size = 0.5)+ # 添加置信椭圆
  # 自动提取主成分解释度进行绘图
  labs(x=paste0("PCoA1 ",x_label,"%"),  
       y=paste0("PCoA2 ",y_label,"%"))+
  scale_fill_manual(values=c("#00c4ff", "#616161"))+
  theme(legend.position = "none") # 设置图例位置，此处为相对位置
ggsave("PCoA_BW.tiff", width = 8, height = 8, units = "cm", dpi = 300)